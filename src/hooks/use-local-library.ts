...
const ensureLocalUniqueCategory = (
	localLibrary: LocalLibrary,
	id: string,
	targetCategory: LocalLibraryCategory,
): LocalLibrary => {
	// Tạo bản sao để tránh thay đổi trực tiếp
	const newLocalLibrary = { ...localLibrary };

	// Xóa ID khỏi tất cả các danh mục
	(Object.keys(newLocalLibrary) as LocalLibraryCategory[]).forEach(
		(category) => {
			newLocalLibrary[category] = newLocalLibrary[category].filter(
				(mangaId) => mangaId !== id,
			);
		},
	);
...
export function useLocalLibrary() {
	...
	const addToLocalCategory = (id: string, category: LocalLibraryCategory) => {
		setLocalLibrary((current) =>
			ensureLocalUniqueCategory(current, id, category),
		);
	};
	...
	const removeFromLocalLibrary = (id: string) => {
		setLocalLibrary((current) => {
			...
			Object.keys(newLocalLibrary).forEach((category) => {
				newLocalLibrary[category as LocalLibraryCategory] = newLocalLibrary[
					category as LocalLibraryCategory
				].filter((mangaId) => mangaId !== id);
			});
			return newLocalLibrary;
		});
	};
